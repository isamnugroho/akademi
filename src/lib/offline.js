...
(function() {
  ...
  defaultOptions = { 
    checks:{
      xhr:{
        url:function() {
          return "http://www.vitaminedz.com/photos/49/02-49565-front-de-mer-a-oran.jpg?_=ANJIM" + new Date().getTime();
        },
        timeout:5e3,
        type:"HEAD"
      },
      image:{
        url:function() {
          return "http://www.vitaminedz.com/photos/49/02-49565-front-de-mer-a-oran.jpg?_=ASU" + new Date().getTime();
        }
      },
      active:"xhr"
    },
  ...
  RETRY_TEMPLATE = '<a href class="offline-ui-retry"></a>', createFromHTML = function(html) {
    ...
    var button, handler;
    return el = createFromHTML(TEMPLATE), document.getElementById("demo_offline").appendChild(el), null != Offline.reconnect && Offline.getOption("reconnect") && (el.appendChild(createFromHTML(RETRY_TEMPLATE)), 
